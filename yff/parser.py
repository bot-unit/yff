...
class YahooFinanceParser:
    ...
    def _parse_earnings_trend(data):
        ...
        for t in data.get("trend", []):
            try:
                ...
                trend["earningsEstimateAvg"] = t["earningsEstimate"]["avg"].get(
                    "raw", 0
                )
                trend["earningsEstimateLow"] = t["earningsEstimate"]["low"].get(
                    "raw", 0
                )
                trend["earningsEstimateHigh"] = t["earningsEstimate"]["high"].get(
                    "raw", 0
                )
                trend["earningsEstimateYearAgoEps"] = t["earningsEstimate"][
                    "yearAgoEps"
                ].get("raw", 0)
                trend["earningsEstimateGrowth"] = t["earningsEstimate"]["growth"].get(
                    "raw", 0
                )
                trend["revenueEstimateAvg"] = t["revenueEstimate"]["avg"].get("raw", 0)
                trend["revenueEstimateLow"] = t["revenueEstimate"]["low"].get("raw", 0)
                trend["revenueEstimateHigh"] = t["revenueEstimate"]["high"].get(
                    "raw", 0
                )
                ...
                trend["epsRevisionsUpLast7days"] = t["epsRevisions"]["upLast7days"].get(
                    "raw", 0
                )
                trend["epsRevisionsUpLast30days"] = t["epsRevisions"][
                    "upLast30days"
                ].get("raw", 0)
                trend["epsRevisionsDownLast30days"] = t["epsRevisions"][
                    "downLast30days"
                ].get("raw", 0)
                trends.append(trend)
            except KeyError:
    ...
    _quote_summary_parsers = {
        ...
        "incomeStatementHistory": lambda data: YahooFinanceParser._parse_income_statement_history(
            data
        ),
        "incomeStatementHistoryQuarterly": lambda data: YahooFinanceParser._parse_income_statement_history(
            data
        ),
        "balanceSheetHistory": lambda data: YahooFinanceParser._parse_balance_sheet_history(
            data
        ),
        "balanceSheetHistoryQuarterly": lambda data: YahooFinanceParser._parse_balance_sheet_history(
            data
        ),
        "cashFlowStatementHistory": lambda data: YahooFinanceParser._parse_cashflow_statement_history(
            data
        ),
        "cashFlowStatementHistoryQuarterly": lambda data: YahooFinanceParser._parse_cashflow_statement_history(
            data
        ),
        ...
        "upgradeDowngradeHistory": lambda data: YahooFinanceParser._parse_upgrade_downgrade_history(
            data
        ),
        "institutionOwnership": lambda data: YahooFinanceParser._parse_institution_ownership(
            data
        ),
        "fundOwnership": lambda data: YahooFinanceParser._parse_fund_ownership(data),
        "majorDirectHolders": lambda data: YahooFinanceParser._parse_major_direct_holders(
            data
        ),
        "majorHoldersBreakdown": lambda data: data,  # not need to parse
        "insiderTransactions": lambda data: YahooFinanceParser._parse_insider_transactions(
            data
        ),
        ...
        "earningsHistory": lambda data: YahooFinanceParser._parse_earnings_history(
            data
        ),
        ...
        "recommendationTrend": lambda data: YahooFinanceParser._parse_recommendation_trend(
            data
        ),
        ...
        try:
            ...
            if "events" in top_level["result"][0]:
                ...
                if "dividends" in events:
                    df.dividends = YahooFinanceParser._parse_dividends(
                        events["dividends"]
                    )
                if "splits" in events:
