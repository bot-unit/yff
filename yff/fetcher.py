...
API_URLS = ["https://query1.finance.yahoo.com", "https://query2.finance.yahoo.com"]
...
class YahooFinanceAsyncFetcher(metaclass=SingletonMeta):
        ...
        try:
            ...
            if e.status == 401 and with_crumb and first_attempt:
                ...
                return await self._get_json(
                    url, params=params, with_crumb=with_crumb, first_attempt=False
                )
            else:
                raise e
        except aiohttp.ClientError as e:
    ...
    async def _get_yahoo_cookies(self):
        ...
        if resp.url.host == "consent.yahoo.com":
            ...
            await self._session.post(
                f"https://consent.yahoo.com/v2/collectConsent?sessionId={sessionId}",
                data=data,
                allow_redirects=True,
            )
    ...
    async def get_spark(
        self, symbols: str | list, range_: str = "1d", interval: str = "5m"
    ):
        params = dict()
    ...
    async def get_quote_summary(
        self, symbol: str, modules: list = None
    ):  # requires crumb
        params = dict()
...
class YahooFinanceSyncFetcher(metaclass=SingletonMeta):
    ...
    def _post(self, url: str, data: dict = None):
        resp = self._session.post(
            url, data=data, timeout=self._timeout, headers={"User-Agent": USER_AGENT}
        )
        return resp
